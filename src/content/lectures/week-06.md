...
CAD I is the three-dimensional model you build this week: it is the thing the ledger will read a mass line out of from now on, replacing an estimate with a measured value. Week 7 covers the structure that carries this same aircraft—the spar that the tail hangs from, and the joiner that feeds the loads into the wing root.
